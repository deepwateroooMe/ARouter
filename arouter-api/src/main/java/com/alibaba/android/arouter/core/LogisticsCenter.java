...
public class LogisticsCenter {
     ...
     */
    private static void loadRouterMap() { // 核心加载方法：这个，就是【编译、自动化插件 arouter-auto-register】的触发机关方法 ScanSetting 类里
        registerByPlugin = false;

        // 【TODO】：下面的注解，没能看懂。没能明白，【这个方法】里，是什么【registerByPlugin】的值更新，就触发了、自动生成注册代码的？？？
        // 【TODO】：把这个方法，如何触发、自动注册的原理，前因后果，看懂
    ...
    public synchronized static void init(Context context, ThreadPoolExecutor tpe) throws HandlerException {
        ...
        try {
            ...
            loadRouterMap(); // 仅只，更改 registerByPlugin 的值，为 false
            if (registerByPlugin) {
                logger.info(TAG, "Load router map by arouter-auto-register plugin.");
            } else { // 亲爱的表哥的活宝妹，认为：真正触发、自动生成注册代码的逻辑，在这个分支里。【TODO】：不对！这里，明明就没有什么、自动生成注册代码的逻辑与过程！！
                ...
                if (ARouter.debuggable() || PackageUtils.isNewVersion(context)) {
                    ...
                    if (!routerMap.isEmpty()) { // 多进程或多模块应用环境下，字典已经存在；或 debugging 时，或应用版本升级后，重新扫描更新时，字典，已经存在
                        // 多进程？不像、不对？
                        // 或多模块应用环境下，【线程池、多线程】上下文环境，共同操作 routerMap 时，拿到一个，当前线程、后续会异步提交的、当前编辑字典的 Editor 索引
                        // apply(): 异步提交你的改变，来替换当前 SharedPreferences 对象中的任何内容。
                    ...
                    // MODE_PRIVATE	为默认操作模式，其中创建的文件只能被调用应用程序访问，既只能被应用本身访问，在该模式下，写入的内容会覆盖原文件的内容。
                    routerMap = new HashSet<>(context.getSharedPreferences(AROUTER_SP_CACHE_KEY, Context.MODE_PRIVATE).getStringSet(AROUTER_SP_KEY_MAP, new HashSet<String>()));
                }
                // 要把、这个分支里，接下来的、后续逻辑，看完：
                logger.info(TAG, "Find router map finished, map size = " + routerMap.size() + ", cost " + (System.currentTimeMillis() - startInit) + " ms."); // 查找、导航字典：做完了，耗时。。。
                startInit = System.currentTimeMillis();
                // 真正的、注册逻辑、过程：？？？还是不像。那个编译插件，先干过活儿、至少、把自动的结果，已经存在了内存中【TODO】：
